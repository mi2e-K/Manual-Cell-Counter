...
from .datatypes import (
    ChannelMode, MarkerType, ToolMode,
...
class FluoroAnalyzer(QMainWindow):
    ...
    def delete_cell_type(self, name: str):
        ...
        # Check if there are markers using this type
        markers_with_type = [m for m in self.cell_markers if m.cell_type == name]
        if markers_with_type:
            reply = QMessageBox.question(
                self, "Delete Cell Type",
                f"There are {len(markers_with_type)} markers using '{name}'.\n"
                "Deleting this type will also remove these markers.\n\nContinue?",
    ...
    def refresh_markers(self):
        ...
        for marker in self.cell_markers:
            ...
            text_item.setPos(pos.x() + size/2 + 2, pos.y() - size/2 - 2)
    ...
    def export_csv(self):
        ...
        if not markers_in_roi:
            QMessageBox.warning(self, "No Data", "No markers within ROIs to export.")
            return
            ...
            for cell_type in self.cell_types:
                for roi in self.rois:
                    if roi.closed:
                        ...
                        if count > 0:
                            if cell_type not in data["summary"]:
    ...
    def export_all(self):
        ...
        if not markers_in_roi and not self.rois:
        ...
        if self.export_csv_checkbox.isChecked() and markers_in_roi:
